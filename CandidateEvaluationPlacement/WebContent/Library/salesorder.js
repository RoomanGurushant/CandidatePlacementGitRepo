    ...
    $('#dialog_link').click(function(){
        ...
        $.ajax({
            ...
            success: function(data){
                $('#dialog-modal').html('<iframe src="../account/account-contact-list.jsp?group=select_so_contact" width="100%" height="100%" frameborder=0></iframe>');
            }
        });
...
function ShowNameHint(){
				...
				showHint('salesorder-check.jsp?contact_fname='+fname+'&contact_lname='+lname+'&session_id='+session_id,'so_details');
				}
...
function SelectContact(contact_id, contact_name, account_id, account_name){
	...
	var selected_account = document.getElementById("span_acct_id").value;
        var so_account = document.getElementById("acct_id").value;
	...
	if(status=='Add' && so_account==0){
		...
		document.getElementById("span_so_contact_id").innerHTML = contact_link;
		document.getElementById("span_so_account_id").innerHTML = account_link;
	}
	GetContactLocationDetails(contact_id);
    var session_id = document.getElementById("txt_session_id").value;
    showHint('salesorder-details.jsp?status='+status+'&session_id='+session_id+'&list_cartitems=yes',session_id,'so_details');
...
function GetContactLocationDetails(so_contact_id){
        showHint('../sales/salesorder-check.jsp?so_contact_id='+so_contact_id,so_contact_id,'so_contact');
	...
	function PopulateContactLocationDetails(){
        ...
        document.getElementById('txt_so_bill_address').value=contact_arr[0];
        document.getElementById('txt_so_bill_city').value=contact_arr[1];
        document.getElementById('txt_so_bill_pin').value=contact_arr[2];
        document.getElementById('txt_so_bill_state').value=contact_arr[3];
	...
	function CopyBillingAddress(){
		var billing_address = document.getElementById("txt_so_bill_address").value;
		var billing_city = document.getElementById("txt_so_bill_city").value;
		var billing_pin = document.getElementById("txt_so_bill_pin").value;
		var billing_state = document.getElementById("txt_so_bill_state").value;
		document.getElementById("txt_so_ship_address").value = billing_address;
		document.getElementById("txt_so_ship_city").value = billing_city;
		document.getElementById("txt_so_ship_pin").value = billing_pin;
		document.getElementById("txt_so_ship_state").value = billing_state;
		}
      ...
      function CopyContactAddress(){
        ...
        document.getElementById("txt_so_bill_address").value = contact_address1;
        if(contact_city!='Select'){
            document.getElementById("txt_so_bill_city").value = contact_city;
        }
        document.getElementById("txt_so_bill_pin").value = contact_pin;
        if(contact_state!='Select'){
            document.getElementById("txt_so_bill_state").value = contact_state;
        }
    }


//functions for salesorder-details.jsp

//For Listing Cart Items
function list_cart_items(){
	var status = document.getElementById("txt_status").value;
    var session_id = document.getElementById("txt_session_id").value;
    showHint('salesorder-details.jsp?status='+status+'&session_id='+session_id+'&list_cartitems=yes','so_details');
}

//For Updating cart item
function update_cart_item(cart_id){
    showHint('salesorder-item.jsp?cart_id='+cart_id+'&update_cartitem=yes',cart_id,'so_item');
}

//For Deleting cart item
function delete_cart_item(cart_id){
    var session_id = document.getElementById("txt_session_id").value;
    showHint('salesorder-details.jsp?cart_id='+cart_id+'&session_id='+session_id+'&delete_cartitem=yes',cart_id,'so_details');
...
//close functioms for salesorder-detals.jsp


/// functions for salesorder items
function SOItemSearch()
{
...
function ItemDetails(item_id, item_type_id, item_name, qty, base_price, price, price_variable, disc, tax_id, tax, tax_name, item_serial, cart_item_serial, cart_id, mode){
    ...
    var emp_price_update = document.getElementById("emp_so_priceupdate").value;
    var emp_disc_update = document.getElementById("emp_so_discountupdate").value;
    if(mode=='add'){
	...
	if(item_type_id=='1'){
		showHint('salesorder-details.jsp?cart_item_id='+item_id+'&mode='+mode+'&cart_id='+cart_id+'&configure=yes','configure-details')
		}else{
			document.getElementById("configure-details").innerHTML = "";
			}
...
function AddCartItem(){
    ...
    showHint('salesorder-details.jsp?status='+status+'&session_id='+session_id+'&cart_item_id='+cart_item_id+'&cart_qty='+cart_qty+'&cart_item_serial='+cart_item_serial+'&cart_price='+cart_price+'&cart_discount='+cart_discount+'&cart_tax='+cart_tax+'&cart_tax_id='+cart_tax_id+'&cart_tax_rate='+cart_tax_rate+'&cart_total='+cart_total+'&rowcount=yes&add_cartitem=yes','so_details');
	if(item_type_id=='1'){
...
function AddConfiguredItems(){
	...
	for(var i=1;i<=group_count;i++){
		...
		if(item.checked==true){
			...
			var session_id = document.getElementById("txt_session_id").value;
			showHint('salesorder-details.jsp?status='+status+'&session_id='+session_id+'&cart_item_id='+opitem_item_id+'&cart_option_group='+opitem_gp_name+'&cart_qty='+opitem_qty+'&cart_item_serial='+opitem_serial+'&cart_price='+opitem_price+'&cart_id='+cart_id+'&rowcount=no&add_cartitem=yes','so_details');
			}
		}
		}
	if(mode=='update'){
...
function UpdateCartItem(){
    ...
    showHint('salesorder-details.jsp?status='+status+'&session_id='+session_id+'&cart_id='+cart_id+'&cart_item_id='+cart_item_id+'&cart_qty='+cart_qty+'&cart_item_serial='+cart_item_serial+'&cart_price='+cart_price+'&cart_discount='+cart_discount+'&cart_tax='+cart_tax+'&cart_tax_id='+cart_tax_id+'&cart_tax_rate='+cart_tax_rate+'&cart_total='+cart_total+'&update_cartitem=yes','so_details');
	if(item_type_id=='1'){
		setTimeout('AddConfiguredItems()',200);
		}
    document.getElementById("mode_button").innerHTML = ' ';
...
function LoadPayment(){
    ...
    var total_so = document.getElementById("txt_so_grandtotal").value;
    var total_payment = document.getElementById("total_payment").innerHTML;
    var balance = parseFloat(total_so)-parseFloat(total_payment);
    document.getElementById("balance").innerHTML = balance;
}
// end of function of salesorder items
