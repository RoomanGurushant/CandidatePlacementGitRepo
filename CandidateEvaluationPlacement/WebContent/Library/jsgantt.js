...
JSGantt.GanttChart =  function(pGanttVar, pDiv, pFormat)
{
   ...
   this.Draw = function()
   {
      ...
      var vNameWidth = 180;	
      var vStatusWidth = 70;
      var vLeftWidth = 0 + 220 + 65 + 65 + 65 + 65 + 40;
	 ...
	 // Process all tasks preset parent date and completion %
         ...
         vMainTable =
            '<TABLE id=theTable cellSpacing=0 cellPadding=0 border=1 width=100%><TBODY><TR>' +
            '<TD vAlign=top>';
	   ...
	   // DRAW the Left-side of the chart (names, resources, comp%)
         vLeftTable = 
            '<DIV class=scroll2 id=leftside style="width:0' + vLeftWidth + 'px"><TABLE bgcolor=cyan cellSpacing=0 cellPadding=0 border=1 width=100%><TBODY>' +
		'<TR height=20px><TD colspan=7 align=left style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 11px; BORDER-LEFT: #efefef 1px solid; height=20px">';
		...
		if (vFormatArr.join().indexOf("hour")!=-1) { 
            	if (vFormat=='hour') vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" VALUE="hour" checked>Hour';
                    ...
                    vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" onclick=JSGantt.changeFormat("hour",'+vGanttVar+'); VALUE="hour">Hour';
		}
			
		if (vFormatArr.join().indexOf("day")!=-1) { 
            	if (vFormat=='day') vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" VALUE="day" checked>Day';
            	else                
                    vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" onclick=JSGantt.changeFormat("day",'+vGanttVar+'); VALUE="day">Day';
		}
			
		if (vFormatArr.join().indexOf("week")!=-1) { 
            	if (vFormat=='week') vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" VALUE="week" checked>Week';
            	else                
                    vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" onclick=JSGantt.changeFormat("week",'+vGanttVar+') VALUE="week">Week';
		}
			
		if (vFormatArr.join().indexOf("month")!=-1) { 
            	if (vFormat=='month') vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" VALUE="month" checked>Month';
            	else                
                    vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" onclick=JSGantt.changeFormat("month",'+vGanttVar+') VALUE="month">Month';
		}
		...
		if (vFormatArr.join().indexOf("quarter")!=-1) { 
            	if (vFormat=='quarter') vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" VALUE="quarter" checked>Quarter';
                    ...
                    vLeftTable += '<INPUT TYPE=RADIO NAME="radFormat" onclick=JSGantt.changeFormat("quarter",'+vGanttVar+') VALUE="quarter">Quarter';
		}
...
//            vLeftTable += '<INPUT TYPE=RADIO NAME="other" VALUE="other" style="display:none"> .';
		vLeftTable += '</TD></TR>';
         ...
         vLeftTable +=
            ...
            //'<TD style="BORDER-TOP: #efefef 1px solid; WIDTH: 15px; HEIGHT: 19px"></TD>' +
            '<TD colspan=2 style="BORDER-TOP: #efefef 1px solid; align=center; WIDTH: ' + vNameWidth + 'px; HEIGHT: 19px; TEXT-ALIGN: center;"><b>Task</b></TD>' ;

         if(vShowRes ==1) vLeftTable += '  <TD style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 120px; HEIGHT: 19px" align=center nowrap><b>Resource</b></TD>' ;
         if(vShowDur ==1) vLeftTable += '  <TD style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 19px" align=center nowrap><b>Duration</TD>' ;
         if(vShowComp==1) vLeftTable += '  <TD style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 19px" align=center nowrap><b>% Comp.</b></TD>' ;
         if(vShowStartDate==1) vLeftTable += '<TD style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 19px" align=center nowrap><b>Start Date</b></TD>' ;
         if(vShowEndDate==1) vLeftTable += '<TD style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 19px" align=center nowrap><b>End Date</b></TD>';
         ...
         vLeftTable += '</TR>';
            ...
            for(i = 0; i < vTaskList.length; i++)
            {
               ...
               vID = vTaskList[i].getID();
  		      ...
  		      if(vTaskList[i].getVisible() == 0) 
                  ...
                  '  <TD colspan=2 class=gname style="WIDTH: ' + vNameWidth + 'px; HEIGHT: 19px; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px;" nowrap><NOBR><span style="color: #aaaaaa">';
               ...
               vLeftTable += 
                  '<span onclick=JSGantt.taskLink("' + vTaskList[i].getLink() + '",600,700); style="cursor:pointer"> ' + vTaskList[i].getName() + '</span></a></NOBR></TD>';

               if(vShowRes ==1) vLeftTable += '<TD class=gname style="WIDTH: 60px; HEIGHT: 19px; TEXT-ALIGN: left; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR>' + vTaskList[i].getResource() + '</NOBR></TD>' ;
            ...
            vLeftTable += '</TR></TBODY></TABLE></DIV></TD>';
            ...
            '<TABLE bgcolor=cyan cellSpacing=0 cellPadding=0 border=0 width=100%>'  +
		...
		if(vTmpDate.getHours() == 0) 
         ...
         for(i = 0; i < vTaskList.length; i++)
         {
	       ...
	       else if (vFormat == 'minute')
	       {
            ...
            if( vTaskList[i].getMile()) {
               vRightTable += '<DIV><TABLE width=100% style="position:relative; top:0px; ' + vChartWidth + 'px;" cellSpacing=0 cellPadding=0 border=0>' +
                  '<TR id=childrow_' + vID + ' class=yesdisplay style="HEIGHT: 19px" onMouseover=g.mouseOver(this,' + vID + ',"right","mile") onMouseout=g.mouseOut(this,' + vID + ',"right","mile")>' + vItemRowStr + '</TR></TABLE></DIV>';
               ...
               if( vTaskList[i].getGroup()) {
                  vRightTable += '<DIV><TABLE style="position:relative; top:0px; width: ' + vChartWidth + 'px;" cellSpacing=0 cellPadding=0 border=0 >' +
                     '<TR id=childrow_' + vID + ' class=yesdisplay style="HEIGHT: 19px" bgColor=#f3f3f3 onMouseover=g.mouseOver(this,' + vID + ',"right","group") onMouseout=g.mouseOut(this,' + vID + ',"right","group")>' + vItemRowStr + '</TR></TABLE></DIV>';
                  vRightTable +=
                     '<div id=bardiv_' + vID + ' style="position:absolute; top:5px; left:' + Math.ceil(vTaskLeft * (vDayWidth) + 1) + 'px; height: 7px; width:' + Math.ceil((vTaskRight) * (vDayWidth) - 1) + 'px">' +
                       '<div id=taskbar_' + vID + ' title="' + vTaskList[i].getName() + ': ' + vDateRowStr + '" class=gtask style="background-color:#000000; height: 7px; width:' + Math.ceil((vTaskRight) * (vDayWidth) -1) + 'px;  cursor: pointer;opacity:0.9;">' +
                         '<div style="Z-INDEX: -4; float:left; background-color:#666666; height:3px; overflow: hidden; margin-top:1px; ' +
                               ...
                               'cursor: pointer;" onclick=JSGantt.taskLink("' + vTaskList[i].getLink() + '",600,700);>' +
                           '</div>' +
                        '</div>' +
                  ...
                  vDivStr = '<DIV><TABLE style="position:relative; top:0px; width: ' + vChartWidth + 'px;" cellSpacing=0 cellPadding=0 border=0>' +
                     '<TR id=childrow_' + vID + ' class=yesdisplay style="HEIGHT: 19px" bgColor=#ffffff onMouseover=g.mouseOver(this,' + vID + ',"right","row") onMouseout=g.mouseOut(this,' + vID + ',"right","row")>' + vItemRowStr + '</TR></TABLE></DIV>';
                  vRightTable += vDivStr;
	            ...
	            vRightTable +=
                     '<div id=bardiv_' + vID + ' style="position:absolute; top:4px; left:' + Math.ceil(vTaskLeft * (vDayWidth) + 1) + 'px; height:18px; width:' + Math.ceil((vTaskRight) * (vDayWidth) - 1) + 'px">' +
                        '<div id=taskbar_' + vID + ' title="' + vTaskList[i].getName() + ': ' + vDateRowStr + '" class=gtask style="background-color:#' + vTaskList[i].getColor() +'; height: 13px; width:' + Math.ceil((vTaskRight) * (vDayWidth) - 1) + 'px; cursor: pointer;opacity:0.9;" ' +
                           'onclick=JSGantt.taskLink("' + vTaskList[i].getLink() + '",600,700); >' +
         ...
         vMainTable += vRightTable + '</TD></TR></TBODY></TABLE></BODY></HTML>';
...
JSGantt.taskLink = function(pRef,pWidth,pHeight) 
  {
    if(pWidth)  vWidth =pWidth;  else vWidth =600;
